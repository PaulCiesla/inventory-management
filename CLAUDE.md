...
- **Use the `gh` CLI** for GitHub operations (PRs, issues, reviews, comments)
  - Local branches: `git checkout -b`
- The GitHub MCP server in `.mcp.json` is optional. Do not depend on it and do not prefer it over
  `gh`. It needs `GITHUB_PERSONAL_ACCESS_TOKEN`, which is unset in GitHub Actions, so mandating it
  left the Claude Code Review workflow unable to post its review.
